...
\subsection{原理概述}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Alice和Bob在公开信道上公布各自的基的选择，而不公布测量结果，仅保留基相同的测量结果用于密钥生成。这种随机选择和匹配的机制使得窃听者（Eve）难以获取信息，因为Eve的任何测量都会引入错误，并被Alice和Bob检测到。
		\end{itemize}
		...
		\begin{itemize}
			...
			\item QKD的安全性是信息论上的无条件安全，即只要误码率低于一定阈值（通常为11\%），通过后续的纠错和隐私放大步骤，Alice和Bob可以提取出无条件安全的共享密钥。
		\end{itemize}
	\end{itemize}
...
\end{question}
	...
	检验单光子探测器的探测效率不可以直接使用强光。这是因为单光子探测器（如光电倍增管、雪崩光电二极管、超导纳米线单光子探测器等）设计用于探测极低光强度下的单光子事件。使用强光源可能会导致以下问题：
	...
	\begin{itemize}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\begin{itemize}
					\item 直角基（+基）： $\{|0\rangle, |1\rangle\}$
					\item 对角基（×基）： $\{|+\rangle = \frac{|0\rangle + |1\rangle}{\sqrt{2}}, |-\rangle = \frac{|0\rangle - |1\rangle}{\sqrt{2}}\}$
				\end{itemize}
				\item Alice随机选择这些基态中的一个，来表示经典比特0或1。例如：
				\begin{itemize}
					\item 用+基的$|0\rangle$表示比特0，用+基的$|1\rangle$表示比特1。
					\item 用×基的$|+\rangle$表示比特0，用×基的$|-\rangle$表示比特1。
				\end{itemize}
			\end{itemize}
			...
			\item \textbf{基的选择与测量}：
			\begin{itemize}
				\item Bob在接收到量子态后，随机选择+基或×基对每个光子进行测量。由于Bob的基选择是随机的，测量结果在Alice的基与Bob的基相同时是正确的，在基不同的时候则是随机的。
			\end{itemize}
